...
func (r *SecretReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if _, ok := secret.Labels[LicensingLabel]; !ok {
		log.Info("not a license secret, ignoring")
	...
	for k, v := range license.Grants {
		...
		err := r.Get(ctx, types.NamespacedName{Namespace: secret.Namespace, Name: url[0]}, entitlement)

		if errors.IsNotFound(err) {
			entitlement.Name = url[0]
			entitlement.Namespace = secret.Namespace

			if err = r.Create(ctx, entitlement); err != nil {
				log.Error(err, "error creating entitlement")
		...
		if err = r.Status().Update(ctx, entitlement); err != nil {
			log.Error(err, "error updating entitlement status")
			return ctrl.Result{}, err
		}
	}
